		       ...
		       int irq, const struct sec_mipi_dsim_plat_data *pdata)
{
	...
	dsim->disp_apb = devm_clk_get(dev, "disp-apb");
	if (IS_ERR(dsim->disp_apb)) {
		ret = PTR_ERR(dsim->disp_apb);
		dev_err(dev, "Unable to get disp-apb clock: %d\n", ret);
		return ret;
	}
	...
	clk_prepare_enable(dsim->disp_apb);
	clk_prepare_enable(dsim->clk_cfg);

			   ...
			   struct sk_buff *skb)
{
	...
	if (hw_rx_obj->flags & MCP251XFD_OBJ_FLAGS_FDF) {
		...
		if (priv->can.ctrlmode & CAN_CTRLMODE_CC_LEN8_DLC && dlc > CAN_MAX_DLEN)
			cf->len8_dlc = dlc;

		/* limit the payload length 'len' to CAN_MAX_DLEN */
		cf->len = min_t(u8, (dlc), CAN_MAX_DLEN);
	}
			  ...
			  unsigned int seq)
{
	...
	const struct can_frame *cf = (struct can_frame *)cfd;
	...
	if (cfd->can_id & CAN_RTR_FLAG)
		...
		len_sanitized = can_dlc2len(can_len2dlc(len));
	...
	if (can_is_canfd_skb(skb)) {
		...
		if ((priv->can.ctrlmode & CAN_CTRLMODE_CC_LEN8_DLC) &&
			(cf->len == CAN_MAX_DLEN) &&
...
static int mcp251xfd_probe(struct spi_device *spi)
{
	...
	rx_int = devm_gpiod_get_optional(&spi->dev, "microchip,rx-int",
					 GPIOD_IN);
	if (IS_ERR(rx_int)) {
		dev_err(&spi->dev, "Failed to get RX-INT!\n");
	...
	if (PTR_ERR(reg_vdd) == -ENODEV)
		...
		dev_err(&spi->dev, "Failed to get VDD regulator!\n");
	...
	if (PTR_ERR(reg_xceiver) == -ENODEV)
		...
		dev_err(&spi->dev, "Failed to get Transceiver regulator!\n");
		return PTR_ERR(reg_xceiver);
	}

	clk = devm_clk_get(&spi->dev, NULL);
	if (IS_ERR(clk)) {
		dev_err(&spi->dev, "Failed to get Oscillator (clock)!\n");
		return PTR_ERR(clk);
	}
